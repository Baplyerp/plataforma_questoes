...
def iniciar_corrida(id_assunto):
    ...
    random.shuffle(st.session_state.ls_questoes) 
...
if st.session_state.ls_estado == "GARAGEM":
    ...
    if not st.session_state.ls_respondido:
        ...
        frases_acerto = ["DRS Aberto! Ultrapassagem perfeita.", "Volta mais rápida da sessão!", "Piloto de elite!", "Traçado perfeito!"]
        frases_erro = ["Ihh! Travou os freios e passou reto.", "Toque no muro!", "Rodou na pista!", "Bandeira amarela!"]
        frases_branco = ["Safety Car na pista! Decisão inteligente.", "Recolheu para o box.", "Estratégia conservadora e calculista."]
